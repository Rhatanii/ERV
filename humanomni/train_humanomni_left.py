...
class TrainingArguments(transformers.TrainingArguments):
    ...
    left_padding: bool = field(default=False, metadata={"help": "Use left padding instead of right padding."})
...
class DataCollatorForSupervisedDataset(object):
    ...
    def _left_pad(self, seqs: Sequence[torch.Tensor], pad_value: int) -> torch.Tensor:
        # seqs: list of 1D LongTensor
        max_len = max(s.size(0) for s in seqs)
        out = seqs[0].new_full((len(seqs), max_len), pad_value)
        for i, s in enumerate(seqs):
            out[i, -s.size(0):] = s  # 오른쪽 정렬 → 왼쪽 패딩
        return out

    def _right_pad(self, seqs: Sequence[torch.Tensor], pad_value: int) -> torch.Tensor:
        return torch.nn.utils.rnn.pad_sequence(
            seqs, batch_first=True, padding_value=pad_value
        )
        
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        input_ids_list, labels_list = tuple([inst[k] for inst in instances]
                                            for k in ("input_ids", "labels"))

        use_left = (self.tokenizer.padding_side == "left")

        if use_left:
            input_ids = self._left_pad(input_ids_list, self.tokenizer.pad_token_id)
            labels    = self._left_pad(labels_list,    IGNORE_INDEX)
        else:
            input_ids = self._right_pad(input_ids_list, self.tokenizer.pad_token_id)
            labels    = self._right_pad(labels_list,    IGNORE_INDEX)

        # attention_mask는 pad가 아닌 곳 1
        attention_mask = input_ids.ne(self.tokenizer.pad_token_id).long()

        # 길이 자르기: 왼쪽 패딩이면 "끝 부분"을 남기고 앞을 자른다.
...
def train(attn_implementation=None):
   ...
   # import ipdb; ipdb.set_trace()
    ...
    config.mm_audio_tower = model_args.audio_tower
    ...
    padding_side = "left" if training_args.left_padding else "right"
    print(f"Set tokenizer padding side to {padding_side}")
