...
class HumanOmniMetaForCausalLM(ABC):
    ...
    def encode_audios(self, audios): # (bs=1, 128, 3000)
        audio_features = self.get_model().get_audio_tower()(audios).permute(0, 2, 1).contiguous() #b, t, c -> b, c, t   # torch.Size([bs=1, 1280, 1500])
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if audios is not None:
            ...
            elif len(audios.shape) == 4 and audios.shape[1] != 1:
                reshape_bs = audios.shape[0] * audios.shape[1]
                audios = audios.view(reshape_bs, audios.shape[2], audios.shape[3])
        ...
        if audios is not None and audio_tower is not None:
            # print('Befor encode audio:', audios.shape)
            audio_features = self.encode_audios(audios) # (bs=1, 500, 896)
        new_input_embeds = []
