    ...
    def forward(
       ...
       # audios=kwargs.get('audios', None)
        ...
        if labels is not None:
            ...
            # 유효한 label 위치만 선택 (ignore_index=-100 제외)
            mask = shift_labels.ne(-100)  # True/False mask, shape [batch, seq_len-1]

            # CrossEntropyLoss는 이미 reduction="none"이므로 그대로 사용
            ce_loss = loss_fct(
            ...
            gathered_probs = probs.gather(
                dim=-1,
                index=shift_labels.clamp(min=0).unsqueeze(-1)  # -100 → 0으로 클램핑해서 안전 gather
            ).squeeze(-1)

            # 마스킹된 loss만 사용
            weighted_loss = ce_loss * gathered_probs.detach() * mask
            custom_loss = weighted_loss.sum() #/ mask.sum()
