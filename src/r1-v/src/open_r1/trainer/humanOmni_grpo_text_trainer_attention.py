...
class HumanOmniVLGRPO_TEXT_Attention_Trainer(Trainer):
    """
    ...
    def __init__(
       ...
       # import ipdb;ipdb.set_trace()
        ...
        if processing_class is None:
            processing_class = processing_class = AutoProcessor.from_pretrained("Qwen/Qwen2-VL-2B-Instruct")
    ...
    self, model, input_ids, attention_mask, images, audios, prompts, answer_length, capture_attention: bool = True,
    ...
    offload_to_cpu: bool = False,
        ...
        if capture_attention:
            ...
            layer_idx = len(outputs.attentions) - 1  # 마지막 레이어
            ...
            if prompt_length is None:
                # 전체 길이에서 answer_length와 completion_mask 등으로 추정할 수도 있지만
                # 모호성을 피하려면 compute_loss에서 명시 전달 권장
                prompt_length = last.size(-1) - answer_length

            # 프롬프트로의 attention 통계
            vis_length = 392
            audio_length = 500
            vision_start_idx = 21
            audio_start_idx = vision_start_idx + vis_length + 8

            
            prompt_divisor = prompt_length + torch.arange(1, 1 + answer_length, dtype=last_mean.dtype, device=last_mean.device)
            reason_numerator = torch.arange(1, 1 + answer_length, dtype=last_mean.dtype, device=last_mean.device)
            
            to_prompt = last_mean[:, :, :prompt_length].sum(dim=-1) / (prompt_length / prompt_divisor.unsqueeze(0))  # (B, answer), P
            to_vision = last_mean[:, :, vision_start_idx:vision_start_idx+vis_length].sum(dim=-1) / (vis_length / prompt_divisor.unsqueeze(0))  # (B, answer), vis
            to_audio = last_mean[:, :, audio_start_idx:audio_start_idx+audio_length].sum(dim=-1) / (audio_length / prompt_divisor.unsqueeze(0))  # (B, answer), aud
            to_reason = last_mean[:, :, prompt_length:].sum(dim=-1) / (reason_numerator.unsqueeze(0) / prompt_divisor.unsqueeze(0))  # (B, answer), reason

            attn_pack = {
                "last_mean": last_mean,             # (B, answer, L)
                "to_prompt_sum": to_prompt,     # (B, answer)
                "to_vision_sum": to_vision,     # (B, answer)
                "to_audio_sum": to_audio,         # (B, answer)
                "to_reason_sum": to_reason     # (B, answer)
            }
            # 큰 텐서는 즉시 참조 해제해 메모리 압력 완화
            del last, last_ans, last_mean, to_prompt, to_vision, to_audio, to_reason
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if len(audios) ==1:
            audios = torch.cat(audios, dim=0).unsqueeze(0) # (1, 128, 3000)
        else:
        ...
        audios_repeat = prompt_inputs['audios'].repeat_interleave(self.num_generations, dim=0) # (G, bs=1, 128, 3000) 
        prompts_repeat = {}
        ...
        out= self._get_per_token_logps_video(model, prompt_completion_ids_repeat, attention_mask, images_repeat, audios_repeat, prompts_repeat, answer_length)
        ...
        if self.ref_model is not None:
            with torch.inference_mode():
                if self.ref_model is not None:
                    print('--------------------------ref_model Numgen ----------------------')
                    ref_per_token_logps = self._get_per_token_logps_video(self.ref_model, prompt_completion_ids_repeat, attention_mask, images_repeat, audios_repeat, prompts_repeat, answer_length, capture_attention=False )
                else:
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, PreTrainedModel):
                ...
                if attn_pack is not None:
                    # (B*G, answer)로 맞추기
                    attn_to_prompt_sum_seq = attn_pack["to_prompt_sum"]  # (B*G, answer_len)
                    attn_to_vision_sum_seq = attn_pack["to_vision_sum"]
                    attn_to_audio_sum_seq = attn_pack["to_audio_sum"]
                    attn_to_reason_sum_seq = attn_pack["to_reason_sum"]
                    
                    # 시퀀스 단위 스칼라(토큰 평균)
                    reward_kwargs["attention_prompt_scores"] = attn_to_prompt_sum_seq.detach().float().cpu()
                    reward_kwargs["attention_vision_scores"] = attn_to_vision_sum_seq.detach().float().cpu()
                    reward_kwargs["attention_audio_scores"] = attn_to_audio_sum_seq.detach().float().cpu()
                    reward_kwargs["attention_reason_scores"] = attn_to_reason_sum_seq.detach().float().cpu()
