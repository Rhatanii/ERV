...
from transformers.trainer_utils import get_last_checkpoint  ### RESUME ADD

from math_verify import parse, verify
from utils import  match_emotion_key_DFEW, match_emotion_key_MAFW, match_emotion_EMOBERT
# from utils_reward import think_emotion_keyword_av_attention_reward, think_av_focus_attention_reward, think_modality_regularizer_reward,

from utils_reward import think_step_av_with_neutral_reward, av_format_reward, think_emotion_keyword_attention_reward , think_av_attention_reward, think_reward, think_step_reward, think_half_reward, think_top1_reward, think_with_hint_reward, think_confusion_reward, think_emotion_label_reward, think_step_with_neutral_reward, chain_reward, accuracy_reward, consistency_reward


from open_r1.trainer import Qwen2VLGRPOTrainer, Qwen2VLGRPOVLLMTrainer, HumanOmniVLGRPO_TEXT_Attention_AU_Trainer
...
class GRPOScriptArguments(ScriptArguments):
    ...
    select_layer_idx: Optional[Union[int, str]] = field(
        default=18,
        metadata={"help": "Which layer's attention to use for reward functions"},
    )
...
reward_funcs_registry = {
    "think": think_reward,
    "av_format": av_format_reward,
    "think_av_with_neutral": think_step_av_with_neutral_reward,
    # "think_av_focus_attention": think_av_focus_attention_reward,
    # "think_emotion_keyword_av_attention": think_emotion_keyword_av_attention_reward,
    "think_emotion_keyword_attention": think_emotion_keyword_attention_reward,
    # "think_modality_regularizer": think_modality_regularizer_reward,
...
def main(script_args, training_args, model_args):
   ...
   # dataset = load_dataset(script_args.dataset_name, name=script_args.dataset_config)
    ...
    trainer_cls = HumanOmniVLGRPO_TEXT_Attention_AU_Trainer 
    ...
    trainer = trainer_cls(
        ...
        select_layer_idx = script_args.select_layer_idx
    )
    ...
    if resume_checkpoint is not None:
        trainer.train(resume_from_checkpoint=resume_checkpoint)  ### RESUME ADD
    else:
        trainer.train()
        
    # Save and push to hub
    trainer.save_model(training_args.output_dir)
    trainer.save_state()  ### RESUME ADD: includes optimizer/scheduler/rng for future resume
